...
export class DangKyPhongController {
  ...
  async getPage(
    ...
    const { total, data } = this.dangKyPhongService.getPagingComponent(
      condition,
      option
    );
    const result = await Promise.all([
      total,
      data.populate("idSinhVien").populate("idPhong"),
    ]);
  ...
  @Get("many")
  @ApiCondition()
  async getMany(@QueryCondition(DangKyPhongConditionDto) condition: any) {
    const data = await this.dangKyPhongService
      .get(condition)
      .populate("idSinhVien")
      .populate("idPhong");
  ...
  @Post()
  async create(@Body() body: CreateDangKyPhongDto) {
